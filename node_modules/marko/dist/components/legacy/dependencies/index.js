...
function getRootDeps(template, context) {
    if (production && template.ad_) {
        return template.ad_;
    }

    attachDepsAndComponentsToTemplate(template, context);

    var deps = template.ad_ = Object.keys(template.ae_).map(key => template.ae_[key]);
...
function attachDepsAndComponentsToTemplate(target, context) {
    ...
    if (production && target.ae_) return;
    ...
    if (meta.tags) {
        meta.tags.forEach(tagPath => {
            ...
            if (tag.ae_) Object.assign(deps, tag.ae_);
