...
const SAFE_TO_PRINT_ON_ONE_LINE = ["If", "Else", "ElseIf", "ForStatement", "WhileStatement", "FunctionDeclaration"];

module.exports = function migrate(el, context) {
    if (el.hasAttribute("marko-init")) {
        if (el.tagName !== "script") {
            context.addError('The "marko-init" attribute should only be used on the <script> tag');
            return;
        }
        ...
        if (el.tagName !== "script") {
            context.addError('The "template-helpers" attribute should only be used on the <script> tag');
            return;
        }
    ...
    parsed.forEachChild(function insertStaticOrImport(child) {
        if (!child.type) {
            ...
            child.declarations.forEach(declaration => {
                ...
                el.insertSiblingBefore(isRequireCall(init) ? toImportTag(id, init.args[0].value, context) : toStaticTag(builder.vars([declaration]), context));
            });
        } else {
            el.insertSiblingBefore(isRequireCall(child) ? toImportTag(undefined, child.args[0].value, context) : toStaticTag(child, context));
        }
    });
...
function isRequireCall(node) {
    return node.type === "FunctionCall" && node.callee.name === "require" && node.args.length === 1 && node.args[0].type === "Literal";
}