...
module.exports = function migrator(elNode, context) {
    ...
    const scopeExpression = elNode.getAttributeValue("scope") || (methodExpression ? provider : null);
    const argAssignments = elNode.attributes.filter(attr => attr.name && attr.name.startsWith("arg-")).map(attr => {
    ...
    if (argExpression) {
        if (argAssignments.length) {
            ...
            if (argExpression.type !== "ObjectExpression") {
                argExpression = builder.binaryExpression(argExpression, "||", builder.objectExpression([]));
            }

            elNode.insertSiblingBefore(builder.scriptlet({
                value: printJS(builder.vars({
                    arg: argExpression
                }), context)
            }));
            ...
            argAssignments.forEach(assignment => {
                elNode.insertSiblingBefore(builder.scriptlet({
                    value: printJS(builder.assignment(builder.memberExpression(argIdentifier, builder.literal(assignment[0]), true), assignment[1], "="), context)
                }));
            });
        ...
        argExpression = builder.objectExpression(argAssignments.reduce((obj, parts) => {
            obj[parts[0]] = parts[1];
            return obj;
        }, {}));
    }
    ...
    if (argExpression || scopeExpression) {
        ...
        provider = builder.functionCall(builder.memberExpression(provider, builder.identifier("bind")), callArgs);
    }
    ...
    if (renderPlaceholderAttr && !placeholderBody) {
        context.deprecate('The "renderPlaceholder" attribute on the "<await>" tag is deprecated and will be removed in a future version of Marko.');
        placeholderBody = [buildDynamicTag(renderPlaceholderAttr, context)];
    }

    if (renderTimeoutAttr && !timeoutBody) {
        context.deprecate('The "renderTimeout" attribute on the "<await>" tag is deprecated and will be removed in a future version of Marko.');
        timeoutBody = [buildDynamicTag(renderTimeoutAttr, context)];
    }

    if (renderErrorAttr && !errorBody) {
        context.deprecate('The "renderError" attribute on the "<await>" tag is deprecated and will be removed in a future version of Marko.');
    ...
    if (placeholderBody) {
        elNode.appendChild(builder.htmlElement("@placeholder", undefined, placeholderBody));
    }

    if (timeoutBody) {
        const originalErrorBody = errorBody;
        errorBody = [builder.htmlElement("if", undefined, timeoutBody, 'err.name === "TimeoutError"')];
